...
import useComida from '../services/useComida';
import ProductGrid from '../components/product/ProductGrid';
import SkeletonGrid from '../components/product/SkeletonGrid';

const Comida = () => {
    const { comida, loading, error } = useComida();
    ...
    const categories = useMemo(
        () => ['Todos', ...new Set(comida.map(p => p.categoria))],
        [comida]
    );
    ...
    const filteredProducts = useMemo(
        () => comida.filter(p => p.nombre && p.precio && p.imgurl),
        [comida]
    );
