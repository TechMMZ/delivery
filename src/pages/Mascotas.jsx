...
import useMascota from '../services/useMascota';
import ProductGrid from '../components/product/ProductGrid';
import SkeletonGrid from '../components/product/SkeletonGrid';

const Mascotas = () => {
    const { mascota, loading, error } = useMascota();
    ...
    const categories = useMemo(
        () => ['Todos', ...new Set(mascota.map(p => p.categoria))],
        [mascota]
    );
    ...
    const filteredProducts = useMemo(
        () => mascota.filter(p => p.nombre && p.precio && p.imgurl),
        [mascota]
    );
